...
func (a *Account) addStreamWithAssignment(config *StreamConfig, fsConfig *FileStoreConfig, sa *streamAssignment, pedantic, recovering bool) (*stream, error) {
	...
	delayedMsgsSoftLimit := delayedMessagesSoftLimit
	if config.DelayedMessagesSoftLimit != 0 {
		delayedMsgsSoftLimit = config.DelayedMessagesSoftLimit
	}
		...
		for i, subj := range cfg.Subjects {
			...
			if !cfg.NoAck {
				for _, namespace := range []string{"$JS.>", "$JSC.>", "$NRG.>"} {
					// Changed from SubjectsCollide to subjectsIsSubsetMatch to be more lenient, allowing to sub to *.something.etc
					if subjectIsSubsetMatch(subj, namespace) {
				...
				if subjectIsSubsetMatch(subj, "$SYS.>") {
	...
	if lseq == 0 && ts == 0 {
		...
		if err == nil && mset.cfg.CheckMessageDependencies {
			...
			for {
				...
				var dMsgErr error
				seq, _, dMsgErr = store.StoreMsg(dmsg.subj, dmsg.hdr, dmsg.msg, 0)
				if dMsgErr != nil {
					mset.srv.Errorf("Failed to store previously delayed message in stream '%s' for subject '%s' from '%s': %v", mset.cfg.Name, dmsg.subj, dmsg.source, dMsgErr)
